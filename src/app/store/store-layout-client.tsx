...
function StoreNavContent({ orgId, storeId }: { orgId: string; storeId: string }) {
    ...
    const handleSearch = useCallback((e: React.FormEvent) => {
        e.preventDefault();
        router.push(searchValue.trim() ? `/store?q=${encodeURIComponent(searchValue.trim())}` : '/store');
    }, [searchValue, router]);
        ...
        <header className="sticky top-0 z-40 border-b border-neutral-800 bg-black/95 backdrop-blur-md">
            <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4">
                ...
                <form onSubmit={handleSearch} className="relative mx-4 flex-1 max-w-md">
                    ...
                    <input type="text" value={searchValue} onChange={(e) => setSearchValue(e.target.value)} placeholder="Mahsulot qidirish..."
                        className="w-full rounded-lg border border-neutral-700 bg-neutral-900 py-2 pl-9 pr-20 text-sm text-white placeholder:text-neutral-500 focus:border-neutral-500 focus:outline-none transition-colors" />
