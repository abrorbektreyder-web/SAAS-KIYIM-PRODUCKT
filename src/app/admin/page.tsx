...
export default function AdminLoginPage() {
    ...
    return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-[#09090b] px-4">
            <div className="pointer-events-none fixed inset-0 bg-gradient-to-br from-neutral-950 via-[#09090b] to-neutral-950" />
            ...
            <div className="relative w-full max-w-sm animate-fade-in">
                ...
                <div className="mb-8 flex flex-col items-center">
                    ...
                    <p className="text-sm text-neutral-500">Do'konlar boshqaruv tizimi</p>
                </div>

                {/* Rol ko'rsatgich */}
                <div className="mb-4 flex items-center justify-center gap-2 rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3">
                    <Shield className="h-5 w-5 text-amber-400" />
                    <span className="text-sm font-medium text-amber-400">Admin boshqaruv paneli</span>
                </div>

                {/* Form */}
                <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 p-8 backdrop-blur-md">
                    <h2 className="mb-1 text-lg font-semibold text-white">Admin kirishi</h2>
                    <p className="mb-6 text-xs text-neutral-500">Do'konlarni boshqarish uchun</p>

                    <form onSubmit={handleLogin} className="space-y-4">
                        <div>
                            <label className="mb-1.5 block text-xs font-medium text-neutral-400">Email manzil</label>
                            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required
                                className="w-full rounded-lg border border-neutral-700 bg-neutral-800 px-4 py-2.5 text-sm text-white placeholder:text-neutral-500 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/50 transition-colors"
                                placeholder="admin@hoyr.uz" />
                        </div>
                        <div>
                            <label className="mb-1.5 block text-xs font-medium text-neutral-400">Parol</label>
                            <div className="relative">
                                <input type={showPass ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required
                                    className="w-full rounded-lg border border-neutral-700 bg-neutral-800 px-4 py-2.5 pr-10 text-sm text-white placeholder:text-neutral-500 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/50 transition-colors"
                                    placeholder="••••••••" />
                        ...
                        <button type="submit" disabled={loading}
                            ...
                            {loading ? 'Kirilmoqda...' : '🛡️ Admin sifatida kirish'}
                        </button>
                    </form>
                ...
                <div className="mt-4 flex flex-col gap-2 text-center">
                    ...
                    <Link href="/pos" className="text-xs text-neutral-600 hover:text-neutral-400 transition-colors mt-2">
